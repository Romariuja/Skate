...
public class AllignPlayer : Player_Controller {
    ...
    void Update()
    {
        ...
        // GetComponent<Rigidbody2D>().velocity = transform.forward.normalized * 200;
        ...
        posOffset = new Vector2(transform.position.x + xOffset, transform.position.y + yOffset);
        RaycastHit2D hit = Physics2D.Raycast(posOffset, -Vector2.up, 1 << LayerMask.NameToLayer("Floor"), -2);
        if (hit.collider!=null)
            Debug.Log(hit.collider.name);
        Debug.DrawLine(posOffset, hit.point, Color.green);
